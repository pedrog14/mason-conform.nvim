                ...
                function()
                    return Optional.of_nilable(language_map[formatter_name])
                        :if_not_present(function()
                            notify(
                                ("Could not find conform formatter %q."):format(
                                    formatter_name
                                ),
                        ...
                        :map(function(package_names)
                            package_names = _.filter(function(package_name)
                                return formatter_mapping.package_to_conform[package_name]
                                    ~= nil
                            end, package_names)
                            ...
                            return a.promisify(vim.ui.select)(package_names, {
                                ...
                                format_item = function(package_name)
                                    local conform_name =
                                        formatter_mapping.package_to_conform[package_name]
                                    if registry.is_installed(package_name) then
                                        return ("%s (installed)"):format(
                                            conform_name
                                        )
                                    else
                                        return conform_name
...
_G.mason_conform_completion = {
    available_formatter_completion = function()
        ...
        local completions = sort_deduped(
            _.concat(
                _.keys(language_mapping.get_language_map()),
