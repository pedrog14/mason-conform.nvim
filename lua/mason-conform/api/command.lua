			...
			:or_(function()
				return Optional.of_nilable(language_map[formatter_name])
					:if_not_present(function()
						notify(("Could not find conform formatter %q."):format(formatter_name), vim.log.levels.ERROR)
					end)
...
vim.api.nvim_create_user_command("ConformInstall", function(opts)
	...
	desc = "Install one or more Conform formatters",
	nargs = "*",
	complete = "custom,v:lua.mason_conform_completion.available_server_completion",
})
...
vim.api.nvim_create_user_command("ConformUninstall", function(opts)
	...
	desc = "Uninstall one or more Conform formatters.",
	nargs = "+",
	complete = "custom,v:lua.mason_conform_completion.installed_server_completion",
})

-- selene: allow(global_usage)
_G.mason_conform_completion = {
	available_formatter_completion = function()
		local available_formatters = require("mason-conform").get_available_servers()
	...
	installed_server_completion = function()
		local installed_servers = require("mason-conform").get_installed_servers()
		return table.concat(installed_servers, "\n")
	end,
}

return {
	LspInstall = ConformInstall,
	LspUninstall = ConformUninstall,
}
